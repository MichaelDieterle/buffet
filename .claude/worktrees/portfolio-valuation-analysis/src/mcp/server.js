...
// A fresh McpServer instance is created per request (stateless serverless
// mode) because a single McpServer can only connect to one transport.
// createMcpServer() returns a fully registered server for a request.
...
function createMcpServer() {
  ...
  server.tool(
    ...
    {
      symbol: z.string().min(1).describe('Stock ticker, e.g. AAPL'),
      days: z.number().int().min(5).max(730).optional().describe('Number of days of history (default 90)'),
    },
    async ({ symbol, days }) => {
      const sym = symbol.toUpperCase();
