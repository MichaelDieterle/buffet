...
async function getStockOr404(symbol) {
  return Stock.findOne({ where: { symbol: symbol.toUpperCase() } });
}
...
router.get('/:symbol/export.csv', validate({ params: schemas.stockSymbol }), async (req, res) => {
  try {
    ...
    const stockCsv = `# stock\r\n${rowsToCsv(Object.keys(stockFlat), [stockFlat])}`;
    const blocks = [];
    if (priceHistory.length) {
      ...
      const enhanced = plain.map((p, idx) => ({ ...p, sma_20: ind.sma_20[idx], sma_50: ind.sma_50[idx], ema_12: ind.ema_12[idx], ema_26: ind.ema_26[idx], rsi: ind.rsi[idx], macd: ind.macd[idx], macd_signal: ind.macd_signal[idx], macd_hist: ind.macd_hist[idx] }));
      blocks.push(`# price_history\r\n${rowsToCsv(Object.keys(enhanced[0]), enhanced)}`);
    }
    for (const [name, rows] of [['news', news], ['fundamentals', fundamentals], ['dividends', dividends], ['earnings', earnings], ['calendar_events', events]]) {
      if (!rows.length) continue;
      const flat = rows.map(r => flatten(r.toJSON()));
      blocks.push(`# ${name}\r\n${rowsToCsv(Object.keys(flat[0]), flat)}`);
    }
    const body = [stockCsv, ...blocks].join('\r\n\r\n');
    const safeSymbol = String(stock.symbol).replace(/[^A-Za-z0-9._-]/g, '_').slice(0, 32) || 'stock';
    const filename = `${safeSymbol}-export-${new Date().toISOString().slice(0, 10)}.csv`;
