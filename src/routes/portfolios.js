...
router.post('/import', upload.single('file'), async (req, res) => {
  ...
  const results = [];
  try {
    await new Promise((resolve, reject) => Readable.from(req.file.buffer).pipe(csv()).on('data', data => results.push(data)).on('end', resolve).on('error', reject));
    const holdingsToCreate = results.filter(row => row.Symbol && row.Shares).map(row => ({ ticker: String(row.Symbol).trim().toUpperCase(), quantity: parseNumber(row.Shares), averagePrice: parseNumber(row['Average Price'] || row.Price), currency: row.Currency || 'EUR' })).filter(row => row.ticker && Number.isFinite(row.quantity) && row.quantity > 0 && Number.isFinite(row.averagePrice) && row.averagePrice >= 0);
    if (!holdingsToCreate.length) return res.status(400).json({ error: 'No valid holdings found in CSV' });
  ...
  } catch (err) { console.error('CSV Import Error:', err); res.status(500).json({ error: err.message === 'Only CSV files are supported' ? err.message : 'Failed to import CSV' }); }
});
