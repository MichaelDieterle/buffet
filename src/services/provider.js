...
function stooqCandidates(symbol) {
  ...
  if (s.endsWith('.f')) direct.push(`${s.slice(0, -2)}.de`);
...
async function fetchStooqHistory(symbol, days = 365) {
  ...
  for (const ss of stooqCandidates(symbol)) {
    try {
      ...
      const rows = lines.slice(1).map(x => { const [date, open, high, low, close, volume] = x.split(','); return { date, open: safeNumber(open), high: safeNumber(high), low: safeNumber(low), close: safeNumber(close), volume: safeNumber(volume), adjClose: safeNumber(close) }; }).filter(x => x.date && x.close != null);
...
async function firstSuccessful(symbol, fn) {
  for (const candidate of symbolCandidates(symbol)) {
    try {
      const result = await fn(candidate);
      if (result) return { candidate, result };
    } catch (err) { console.error(`[provider] ${candidate}:`, err.message); }
  }
  return null;
}

async function fetchQuote(symbol) {
  const cached = await firstSuccessful(symbol, candidate => yahoo.fetchQuote(candidate));
  if (cached?.result?.price != null) return { ...cached.result, symbol, dataSymbol: cached.candidate };
  ...
  if (direct?.result?.length) {
    const rows = direct.result, last = rows[rows.length - 1], prev = rows[rows.length - 2];
    const change = prev?.close != null ? last.close - prev.close : 0;
    return { symbol, dataSymbol: direct.candidate, price: last.close, previousClose: prev?.close ?? last.close, change, changePercent: prev?.close ? change / prev.close * 100 : 0, dayHigh: last.high, dayLow: last.low, volume: last.volume, currency: /\.(DE|F)$/i.test(direct.candidate) ? 'EUR' : 'USD', timestamp: `${last.date}T00:00:00.000Z`, exchangeName: /\.DE$/i.test(direct.candidate) ? 'XETRA' : null, marketState: null };
  }

  if (avEnabled()) {
    for (const candidate of symbolCandidates(symbol)) {
      try {
        const q = await httpGetJson(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${encodeURIComponent(candidate)}&apikey=${encodeURIComponent(AV_KEY)}`);
        const g = q?.['Global Quote'];
        if (g?.['05. price']) return { symbol, dataSymbol: candidate, price: safeNumber(g['05. price']), change: safeNumber(g['09. change']), changePercent: safeNumber(String(g['10. change percent'] || '').replace('%', '')), previousClose: safeNumber(g['08. previous close']), open: safeNumber(g['02. open']), dayHigh: safeNumber(g['03. high']), dayLow: safeNumber(g['04. low']), volume: safeNumber(g['06. volume']), currency: /\.(DE|F)$/i.test(candidate) ? 'EUR' : 'USD', timestamp: new Date().toISOString() };
      } catch (e) { console.error(`[provider] AV quote ${candidate}:`, e.message); }
    }
  }

  for (const candidate of symbolCandidates(symbol)) {
    const h = await fetchStooqHistory(candidate, 14);
    if (h.length) { const last = h[h.length - 1], prev = h[h.length - 2]; const change = prev?.close != null ? last.close - prev.close : 0; return { symbol, dataSymbol: candidate, price: last.close, previousClose: prev?.close ?? last.close, change, changePercent: prev?.close ? change / prev.close * 100 : 0, currency: /\.(DE|F)$/i.test(candidate) ? 'EUR' : 'USD', timestamp: last.date }; }
  }
  return null;
}
...
async function fetchFundamentals(symbol) {
  ...
  if (live?.result) return { ...live.result, symbol, dataSymbol: live.candidate };

  // If quoteSummary is unavailable, still expose live valuation fields from the quote.
  const quote = await fetchQuote(symbol);
  if (!quote) return null;
  return { symbol, dataSymbol: quote.dataSymbol || canonical(symbol), peRatio: null, forwardPe: null, pegRatio: null, pbRatio: null, psRatio: null, eps: null, forwardEps: null, marketCap: quote.marketCap ?? null, fiftyTwoWeekHigh: quote.yearHigh ?? null, fiftyTwoWeekLow: quote.yearLow ?? null, lastUpdated: quote.timestamp || new Date().toISOString() };
}

async function fetchNews(symbol) {
  const live = await firstSuccessful(symbol, candidate => yahoo.fetchNews(candidate));
  return live?.result || { all: [], company: [], geopolitics: [] };
}
...
async function fetchHistory(symbol, range = '6mo', interval = '1d', days) {
  ...
  const primary = await firstSuccessful(symbol, candidate => yahoo.fetchHistory(candidate, range, interval).then(rows => rows?.length ? rows : null));
  if (primary?.result) return primary.result;
  const direct = await firstSuccessful(symbol, candidate => fetchDirectYahooHistory(candidate, fallbackDays, interval).then(rows => rows.length ? rows : null));
  ...
  for (const candidate of symbolCandidates(symbol)) { const r = await fetchStooqHistory(candidate, fallbackDays); if (r.length) return r; }
  return [];
}

module.exports = { name: PROVIDER, fallbacks: { alphaVantage: avEnabled(), yahooDirect: true, stooq: true }, fetchQuote, fetchFundamentals, fetchNews, fetchCalendar, fetchHistory, searchSymbol, clearCache, _internals: { stooqCandidates, safeNumber, rangeDays, symbolCandidates, canonical } };
